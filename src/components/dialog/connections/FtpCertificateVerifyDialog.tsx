...
export interface FtpCertificateVerifyRequest {
  requestId: string;
  host: string;
  port: number;
  fingerprint: string;
...
interface FtpCertificateVerifyDialogProps {
  request: FtpCertificateVerifyRequest | null;
  onDone: (requestId: string) => void;
}

export function FtpCertificateVerifyDialog({ request, onDone }: FtpCertificateVerifyDialogProps) {
  ...
  const respond = async (accepted: boolean, remember: boolean) => {
    ...
    setSubmitting(true);
    try {
      await invoke("respond_ftp_certificate", {
      ...
      logger.info({
        ...
        event: accepted ? "ftp_certificate.user_accepted" : "ftp_certificate.user_rejected",
        message: accepted ? "User accepted FTP certificate" : "User rejected FTP certificate",
        ids: { request_id: request.requestId },
      });
    } catch (error) {
      logger.error({
        domain: "security.flow",
        event: "ftp_certificate.response_failed",
        message: "Failed to send FTP certificate response",
        ids: { request_id: request.requestId },
        error,
      });
    }
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-sm">
            ...
            {t("settings.ftpCertificateVerifyTitle")}
          </DialogTitle>
          <DialogDescription className="text-xs">
            {changed
              ? t("settings.ftpCertificateVerifyChanged")
              : t("settings.ftpCertificateVerifyNew")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2 overflow-hidden py-2 text-xs">
          ...
          {changed && (
            <div className="rounded-md border border-destructive/50 bg-destructive/10 p-2 text-[0.6875rem] text-destructive">
              {t("settings.ftpCertificateVerifyWarning")}
            </div>
          )}
        </div>
