...
pub use tables::{
    ...
    NOTES_TABLE, OTP_ACCOUNTS_TABLE, PROXIES_TABLE, RDP_KNOWN_HOSTS_TABLE, FTP_KNOWN_HOSTS_TABLE,
    SETTINGS_TABLE,
    TUNNELS_TABLE,
};
...
pub(crate) fn check_ftp_known_host(
    host: &str,
    port: u16,
    sha256_fingerprint: &str,
) -> AppResult<KnownHostCheck> {
    storage()?.check_ftp_known_host(host, port, sha256_fingerprint)
}

pub(crate) fn upsert_ftp_known_host(
    host: &str,
    port: u16,
    sha256_fingerprint: &str,
    certificate: FtpCertificateMetadata,
) -> AppResult<()> {
    storage()?.upsert_ftp_known_host(host, port, sha256_fingerprint, certificate)
}
