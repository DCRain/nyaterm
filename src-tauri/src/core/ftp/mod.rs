//! FTP file-browser backend file-browser backend (opendal).
...
const FTP_CHUNK_SIZE: usize = 512 * 1024; // 512 KiB per chunk
...
impl FtpManager {
    ...
    pub async fn list_dir(&self, connection_id: &str, path: &str) -> AppResult<Vec<FileEntry>> {
        ...
        let prefix = normalize_ftp_prefix(path);
    ...
    pub async fn create_dir(&self, connection_id: &str, path: &str) -> AppResult<()> {
        let op = self.operator_for(connection_id).await?;
        let key = normalize_ftp_dir_key(path);
    ...
    pub async fn delete(&self, connection_id: &str, path: &str, is_dir: bool) -> AppResult<()> {
        let op = self.operator_for(connection_id).await?;
        if is_dir {
            let prefix = normalize_ftp_dir_key(path);
    ...
    pub async fn upload_file_with_progress(
        ...
        let result: AppResult<u64> = async {
            ...
            let mut buffer = vec![0u8; FTP_CHUNK_SIZE];
    ...
    pub async fn download_directory_with_progress(
        ...
        let op = self.operator_for(connection_id).await?;
        let prefix = normalize_ftp_dir_key(remote_path);
        let entries = op
            .list_with(&prefix)
            .recursive(true)
            .await
...
/// Decrypt FTP connection secrets in place. Public so connection tests and
...
pub fn build_operator_from_connection(connection: &SavedConnection) -> AppResult<Operator> {
    ...
    if let Some(user) = username.as_deref().filter(|v| !v.is_empty()) {
        builder = builder.user(user);
    }
    if let Some(pass) = password.as_deref().filter(|v| !v.is_empty()) {
...
fn normalize_ftp_dir_key(path: &str) -> String {
    normalize_ftp_prefix(path)
}
...
fn map_opendal_error(error: opendal::Error) -> AppError {
    AppError::Config(format!("FTP error: {error}"))
}

/// Resolve connection id from a synthetic FTP workspace session id (`s3:<connectionId>`).
