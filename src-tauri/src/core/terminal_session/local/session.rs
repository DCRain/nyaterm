...
fn pty_session_thread(
    ...
    let elevated = config.as_ref().is_some_and(|cfg| cfg.elevated);
    let (mut cmd, _) = if elevated {
        match wrap_with_elevation(resolved_shell_spec.clone()) {
            Ok(elevated_spec) => build_shell_command_from_spec(&elevated_spec),
            Err(error) => {
                tracing::error!("Failed to build elevated shell command: {}", error);
                let _ = app.emit(
                    &format!("session-error-{}", session_id),
                    format!("Failed to build elevated shell command: {}", error),
                );
                let _ = app.emit(&format!("session-closed-{}", session_id), ());
                rt_handle.block_on(async {
                    manager.remove_session(&session_id).await;
        ...
        build_shell_command_from_spec(&resolved_shell_spec)
    };
