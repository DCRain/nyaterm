...
use crate::config::{ConnectionAuth, ConnectionNetwork, ConnectionType, SavedConnection};
...
/// Uses `agent` auth when an identity file is present (PublicKey), or `agent` otherwise.
fn entry_to_saved_connection(entry: &SshConfigEntry) -> SavedConnection {
    let auth_mode = if entry.identity_file.is_some() {
        "publicKey"
    } else {
        "agent"
    };
    ...
    SavedConnection {
        ...
        description: Some(format!(
            ...
            if entry.proxy_jump.is_some() {
                format!(" (ProxyJump: {})", entry.proxy_jump.as_ref().unwrap())
            } else {
                String::new()
            }
        )),
...
pub fn import_ssh_config_connections(app: &tauri::AppHandle) -> AppResult<usize> {
    ...
    let existing_names: HashSet<&str> =
        cfg.connections.iter().map(|c| c.name.as_str()).collect();
    ...
    for entry in &entries {
        ...
        let conn = entry_to_saved_connection(entry);
...
mod tests {
    ...
    fn parse_simple_config() {
        ...
        let mut hosts = Vec::new();
        let mut visited = HashSet::new();
        parse_string(config, Path::new("/tmp/test"), &mut visited, &mut hosts).unwrap();
    ...
    fn resolve_proxy_jump_chain() {
        ...
        let mut hosts = Vec::new();
        let mut visited = HashSet::new();
        parse_string(config, Path::new("/tmp/chain"), &mut visited, &mut hosts).unwrap();
    ...
    fn resolve_first_match_wins() {
        ...
        let mut hosts = Vec::new();
        let mut visited = HashSet::new();
        parse_string(config, Path::new("/tmp/resolve"), &mut visited, &mut hosts).unwrap();
    ...
    fn list_hosts_skips_wildcards() {
        ...
        let mut hosts = Vec::new();
        let mut visited = HashSet::new();
        parse_string(config, Path::new("/tmp/list"), &mut visited, &mut hosts).unwrap();
